...
def get_model_bin(is_trained=False):
    if not is_trained:
...
def get_model(is_trained=False):
...
def get_occupation_and_label(premise):
    found = [w for w in get_female_occ_list() if w in premise]
    if found:
        return 2,found
    found = [w for w in get_male_occ_list() if w in premise]
    if found:
        return 1, found
    found = [w for w in get_neutral_occ_list() if w in premise]
    if found:
        return 0, found
...
def main():
    ...
    entailment_prob_biasedness('biased_eval_entailment.jsonl')
...
def entailment_prob_biasedness(file):
    model, tokenizer = get_model()
    with open('baseline_prob_diff', 'w', encoding='utf-8') as f:
        with open(file, 'r') as json_file:
            json_list = list(json_file)
            for json_str in json_list:
                result = json.loads(json_str)
                this_hypothesis = result['hypothesis']
                this_premise = result['premise']
                label, occupation = get_occupation_and_label(this_premise)

                entailment_prob_man = 0
                entailment_prob_woman = 0
                ...
                if 'woman' in this_hypothesis:
                    entailment_prob_woman = entailment_prob2
                    entailment_prob_man = entailment_prob1
                else:
                    entailment_prob_woman = entailment_prob1
                    entailment_prob_man = entailment_prob2

                f.write(f'{occupation},{entailment_prob_man},{entailment_prob_woman}' + '\n')
